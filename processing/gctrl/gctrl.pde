...
void draw()
{
  ...
  text("4: set speed to 1.000 inches (1000 mil) per jog", 12, y); y += dy;
  ...
  text("m: JMC test (cuadradillo)", 12, y); y += dy;
  y = height - dy;
...
void keyPressed()
{
  ...
  if (!streaming) {
    ...
    if (key == '$') port.write("$$\n");
    if (key == 'm') port.write("G1X0Y0G1X40Y0G1X40Y40G1X0Y40G1X0M300S30G4P300M300S50\n");
  }
